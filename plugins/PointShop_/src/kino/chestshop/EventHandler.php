...
class EventHandler implements Listener{
	...
	$this->coin = $plugin->getServer()->getPluginManager()->getPlugin("Coins");
	...
	public function handleTransaction(Player $player, Item $itemClicked, Item $itemClickedUsing, SlotChangeAction $inventoryAction) : bool{
		...
		if($nbt->hasTag("ChestShop")){
			...
			if($this->coin->getMoney($player->getName()) < $cost){
				$player->sendMessage(TF::RED."You do not have enough point to purchase ".$itemClicked->getName().".");
				$player->removeWindow($inventoryAction->getInventory());
				return true;
			}
			...
			$player->sendMessage("§l§d[§aTOOL-SHOP§d]".TF::YELLOW."Đã mua ".$itemClicked->getName()." §eSố Lượng:§a ".$itemClicked->getCount().")".TF::RESET.TF::YELLOW." Với giá ".TF::RESET.TF::GOLD.$cost." §bCoin");
